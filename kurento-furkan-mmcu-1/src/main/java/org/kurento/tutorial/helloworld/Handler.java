...
public class Handler extends TextWebSocketHandler
{
  ...
  private MediaPipeline pipeline = null; //= kurento.createMediaPipeline();
  private Composite firstComposite = null;
  Composite secondComposite;
  HubPort secondCompositeHubOut = null;
                                     ...
                                     JsonObject jsonMessage)
  {
    ...
    if(pipeline == null) {
      ...
      pipeline = disUserSession.getMediaPipeline();

      firstComposite = new Composite.Builder(pipeline).build();
      HubPort firstCompositeHubOut = new HubPort.Builder(firstComposite).build();

      secondComposite = new Composite.Builder(pipeline).build();
      HubPort secondCompositeHubIn = new HubPort.Builder(secondComposite).build();
      ...
      PassThrough passThrough = new PassThrough.Builder(pipeline).build();

      firstCompositeHubOut.connect(disUserSession.getWebRtcEndpoint());        //HERE
      //firstCompositeHubOut.connect(secondCompositeHubIn);
      firstCompositeHubOut.connect(passThrough);

      passThrough.connect(secondCompositeHubIn);




      HubPort clientHubIn = new HubPort.Builder(secondComposite).build();       //HERE
      disUserSession.getWebRtcEndpoint().connect(clientHubIn);                  //HERE

      secondCompositeHubOut = new HubPort.Builder(secondComposite).build();
    ...
    HubPort hubPort = new HubPort.Builder(firstComposite).build();
    webRtcEp.connect(hubPort);
    //hubPortSrc.connect(user.getWebRtcEndpoint());
    secondCompositeHubOut.connect(user.getWebRtcEndpoint());
